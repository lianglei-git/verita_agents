...
SYSTEM = """你是 GoalBridge 访谈官，当前执行【步骤 3：信息收集】。
...
请分析：为实现上述目标，还需了解哪些**与目标相关的补充信息**（当前水平、备考时间、弱项等；步骤 1 目标与步骤 2 基础画像中已有字段勿重复追问）。
...
# 步骤 3 状态
{json.dumps({"goal_text": step3.get("goal_text"), "sufficiency": step3.get("sufficiency")}, ensure_ascii=False, indent=2)}
